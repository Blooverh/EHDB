...
export default async function cnAddCPU(titleArr, priceArr, linksArr) {
  ...
  if (
    ...
    console.error(
      "[ERROR] Arrays do not have same length; thus cannot be added to DB",
    );
  ...
  for (let i = 0; i < titleArr.length; i++) {
    ...
    try {
      ...
      if (cpu) {
        ...
        let existingWebsite = cpu.info.find(
        ...
        if (existingWebsite) {
          ...
          console.log(
            `[UPDATED PRICING] ${cpu.brand} - ${cpu.model} -> New Price: ${existingWebsite.currPrice} Old Price: ${existingWebsite.oldPrice}`,
          );
        } else {
          cpu.info.push({
          ...
          console.log(
            `[UPDATED PRICING] ${cpu.brand} - ${cpu.model} -> Price: ${priceInFloat}`,
          );
        }
        ...
        console.log(
          `[SAVED] ${cpu.brand} ${cpu.model} | Slug: ${cpu.slug} | current price: ${priceInFloat} | old price: ${priceInFloat} | link: ${link}`,
        );
      }
    } catch (err) {
