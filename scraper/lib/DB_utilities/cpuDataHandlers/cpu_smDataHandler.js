...
export default async function smAddCPU(titleArr, priceArr, linkArr) {
  ...
  if (
    ...
    priceArr.length !== linkArr.length
  ) {
    throw new Error(
      "Arrays do not have same length hence data cannot be added to Database",
    );
  }
  ...
  for (let i = 0; i < titleArr.length; i++) {
    ...
    try {
      ...
      let existsWebsite = cpu.info.find(
        (entry) => entry.website === "Server Monkey",
      );

      if (existsWebsite) {
        if (existsWebsite.currPrice !== price) {
          existsWebsite.priceHistory.push({
            oldPrice: existsWebsite.currPrice,
            timestamp: new Date(),
          });
        ...
        console.log(
          `[UPDATED PRICING] ${cpu.brand} ${cpu.model} -> Price: ${price}`,
        );
      } else {
        cpu.info.push({
        ...
        console.log(
          `[SAVED ENTRY] ${cpu.brand} ${cpu.model} -> Price: ${price} link: ${link}`,
        );
      }
...
export function extractTitle(title) {
  ...
  let modelWords =
    processorIndex !== -1
      ? words.slice(brandIndex + 1, processorIndex)
      : words.slice(brandIndex + 1);
  ...
  if (modelWords[0] === "Xeon" && /^E\d/.test(modelWords[1])) {
    ...
    if (modelParts) {
      ...
      if (modelParts[2]) {
        // if it does not return null we add it to 3rd index of
