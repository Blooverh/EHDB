...
export default async function xByteAddCPU(titleArr, priceArr, linksArr) {
  ...
  if (titleArr.length !== priceArr.length) {
    console.error(
      "[ERROR ] Both Title and Price array do not have same length",
    );
  ...
  for (let i = 0; i < titleArr.length; i++) {
    ...
    try {
      ...
      // check if xByte entry for this cpu exists and has valid currPrice
      let existsBrand = cpu.info.find(
        (entry) => entry.website === "xByte" && entry.currPrice !== undefined,
      );
      // if exists (truthy value), update current price with new price and old price with price previous to change
      if (existsBrand) {
        if (existsBrand.currPrice !== priceInFloat) {
          existsBrand.priceHistory.push({
            oldPrice: existsBrand.currPrice,
            timestamp: new Date(),
          });
        ...
        if (existsBrand.link === "N/A" || !existsBrand.link) {
          existsBrand.link = link;
        }

        console.log(
          `[UPDATED ENTRY] ${cpu.brand} - ${cpu.model} -> New Price: ${priceInFloat} Old Price: ${existsBrand.oldPrice}`,
        );
      } else {
        cpu.info.push({
          website: "xByte",
        ...
        console.log(
          `[SAVED NEW ENTRY] ${cpu.brand} - ${cpu.model} -> Price: ${priceInFloat} Link: ${link}`,
        );
      }
